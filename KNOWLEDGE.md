...
**Design DNA — `vercel` / agency / clear**
- Layout family: `metric-grid` — Tight grid, one hero metric, mono labels. No four equal stat cards.
...
- Type: One hero metric at display size, labels mono or tracked caps.
- Execute: Black-and-ink precision on this palette. Hairline only. The hero number is 2.5x the body. Secondary figures stay small.
...
- Invariant (unit-test this): Best-before → days left. Use-soon ordered by date. Mark used. No kcal, no slots.
- Empty: The pantry is empty. Scan the first tin.
- Fake that fails: A calorie tracker wearing a kitchen skin.
- Never: OFF is allowed for identity, not for macros.

**Desk `pack_balance` — Pack heatmap**
- Home: Suitcase canvas, drag zones.
- Invariant (unit-test this): fill=Σvol/cap; balanceXY=weight-avg of zones; readiness mixes clip(weight), clip(fill), balance.
- Fake that fails: Packing checklist.
...
**Mini-ref `FoodFlow (identity only)`** (`pantry_expiry`)
- Steal: OFF product-by-barcode and search for identity. Home is the use-soon rail.
- Never: Do not copy Today macros or meal slots. No wrapper.
